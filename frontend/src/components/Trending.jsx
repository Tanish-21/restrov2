  ...
  useEffect(() => {
    ...
    const fetchFoods = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(data.message || 'Failed to fetch trending foods');
        }
      ...
      {foods.map((item) => (
        ...
        >
          ...
          <div className="p-6">
            ...
            <button className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-orange-400 via-orange-500 to-yellow-300 text-black font-headline-md px-8 py-4 rounded-3xl w-full md:w-max hover:shadow-xl hover:shadow-orange-500/20 hover:-translate-y-0.5 transition-all duration-300 active:scale-95 focus:outline-none focus:ring-4 focus:ring-orange-500/30 border border-white/10">
              Add to Cart
            </button>
          </div>
        </div>
